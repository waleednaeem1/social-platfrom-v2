            ...
            <div class="card b-radius--10">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="avatar avatar--md">
                                                <img src="{{ getImage(getFilePath('pages') . '/' . $page->profile_image, getFileSize('pages')) }}"
                                                    alt="@lang('Image')">
                                            </div>
                                        </td>
                                        <td>{{ __($page->page_name) }}</td>
                                        <td>{{ __($page->total_members) }}</td>
                                        <td>{{ __($page->page_type) }}</td>
                                        <td>{{ strLimit(__($page->short_description), 30) }}</td>
                                        <td>{{ $page->created_at}}</td>

                                        <td>
                                            @php
                                                $page->image_with_path = getImage(getFilePath('pages') . '/' . $page->profile_image, getFileSize('pages'));
                                            @endphp

                                            {{-- <button type="button" class="btn btn-sm btn-outline--primary editBtn cuModalBtn"
                                                data-resource="{{ $page }}" data-modal_title="@lang('Edit Group')"
                                                data-has_status="1">
                ...
                @if ($pages->hasPages())
                    <div class="card-footer py-4">
                        @php echo paginateLinks($pages) @endphp
                    </div>
                @endif
            </div><!-- card end -->
        </div>
