        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="mt-3">
                    ...
                    <p>@lang('Email sent') <span class="sent">0</span> @lang('assistants out of') {{ $assistants }} @lang('assistants')</p>
                </div>
            </div>
        </div>
    </div>
...
<script>
    (function($){
        ...
        function postMail(form,skip){
            ...
            $.post("{{ route('admin.assistant.notification.all.send') }}", {
                "subject":subject,
                "_token":_token,
                "skip":skip,
                "message":message
                ...
                if(response.error){
                    ...
                    var rest =  {{ $assistants }} - response.total_sent;
                    var sentPercent = response.total_sent / {{ $assistants }} * 100;
                    if (sentPercent > 100) {
                        sentPercent = 100;
                    }
                    ...
                    if(rest == 0){
                        setTimeout(() => {
                            ...
                            notify('success','Mail sent to all assistants successfully')
                        }, 3000);
                        return false;
                    }
                    setTimeout(()=>{
