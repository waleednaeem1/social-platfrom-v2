            ...
            <div class="card b-radius--10">
                        ...
                        <table class="table table--light">
                            ...
                            <tbody>
                                @forelse($allNews as $news)
                                    <tr>
                                        <td>{{ $allNews->firstItem() + $loop->index }}</td>
                                        <td>
                                            <div class="avatar avatar--md">
                                                <img src="{{ getImage(getFilePath('news') . '/' . $news->image_thumbnail, getFileSize('news')) }}"
                                                    alt="@lang('Image')">
                                            </div>
                                        </td>
                                        <td>{{ __($news->name) }}</td>
                                        <td>{{ __($news->status) }}</td>
                                        {{-- <td>{{ strLimit(__($news->details), 30) }}</td> --}}
                                        <td>{{ $news->created_at}}</td>

                                        <td>
                                            @php
                                                $news->image_thumbnail = getImage(getFilePath('news') . '/' . $news->image_thumbnail, getFileSize('news'));
                                            @endphp

                                            <button type="button" class="btn btn-sm btn-outline--primary editBtn cuModalBtn"
                                                data-resource="{{ $news }}" data-modal_title="@lang('Edit News')"
                                                data-has_status="1">
                ...
                @if ($allNews->hasPages())
                    <div class="card-footer py-4">
                        @php echo paginateLinks($allNews) @endphp
                    </div>
                @endif
            </div><!-- card end -->
        </div>
                        ...
                        <div class="row">
                                        ...
                                        <div class="thumb">
                                            <div class="avatar-preview">
                                                <div class="profilePicPreview"
                                                    style="background-image: url({{ getImage(getFilePath('news'), getFileSize('news')) }})">
                                            ...
                                            <div class="avatar-edit">
                                                ...
                                                <small class="mt-2 ">@lang('Supported files'):
                                                    ...
                                                    @lang('Image will be resized into') {{ getFileSize('news') }} </small>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="col-lg-6">
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea name="heading_content" rows="10" class="form-control" required></textarea>
                                </div>
                                <div class="form-group">
...
@push('script')
    <script>
        (function($) {
            ...
            var placeHolderImage = "{{ getImage(getFilePath('news'), getFileSize('news')) }}";
